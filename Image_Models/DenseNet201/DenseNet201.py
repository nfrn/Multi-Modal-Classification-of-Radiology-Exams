...
def createModelPretrainedRGB():
    base_model = DenseNet201(weights="imagenet",input_shape=(256,256,3), include_top=False)
...
def createModelGrayscale():
    base_model = DenseNet201(weights=None,input_shape=(256,256,1), include_top=False)
...
def trainRGB(model):
    ...
    filepath = "DenseNet-rgb-{epoch:02d}-{val_loss:.2f}.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=False,
                                 mode='min')
    clr = CyclicLR(base_lr=0.0001, max_lr=0.0006, step_size=2000.)
    es = EarlyStopping(monitor="val_loss",mode=min, verbose=1)
...
def generatePredictionsGrayscale(model):
    ...
    np.save("DenseNet_GrayScale_Predictions", array)
