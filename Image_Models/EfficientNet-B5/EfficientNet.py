...
def createModelPretrainedRGB():
    net = EfficientNetB5((256,256,3),weights="imagenet",include_top=False,pooling='avg')
...
def createModelGrayscale():
    net = EfficientNetB5((256,256,1),weights=None,include_top=False,pooling='avg')
    x = Dense(14, kernel_initializer=EfficientNetDenseInitializer())(net.output)
    x = Activation('sigmoid')(x)
    model = Model(net.input,x)
    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy', auc_roc])
...
def trainGrayscale(model):
    ...
    filepath = "EfiNet-grayscale-{epoch:02d}-{val_loss:.2f}.hdf5"
...
def trainRGB(model):
    ...
    filepath = "EfiNet-rgb-{epoch:02d}-{val_loss:.2f}.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=False,
                                 mode='min')
    clr = CyclicLR(base_lr=0.0001, max_lr=0.0006, step_size=2000.)
    es = EarlyStopping(monitor="val_loss", mode=min, verbose=1)
...
def generatePredictionsGrayscale(model):
    ...
    np.save("Eficient_GrayScale_Predictions", array)
...
def generatePredictionsRGB(model):
    ...
    predictions = model.predict_generator(testgenerator, testgenerator.n / 64, verbose=1)
    array = np.array([predictions, testgenerator.labels])
    np.save("Eficient_RGB_Predictions", array)
